...
async fn vector_mode_returns_top_k_known_close_points() {
    ...
    assert!(
        ...
        "alpha must be in top 3: {subjects:?}"
    );
    assert!(
        subjects.contains(&"/notes/alphax"),
        "alphax must be in top 3: {subjects:?}"
    );
    assert!(
        subjects.contains(&"/notes/alphay"),
        "alphay must be in top 3: {subjects:?}"
    );
}
...
async fn vector_mode_without_embedder_returns_error() {
    ...
    let cap_engine = Arc::new(CapEngine::new());
    let server = CtxdMcpServer::new(store, cap_engine, "ctxd://test".to_string());
    let out = server
        .ctx_search(Parameters(SearchParams {
            query: "x".to_string(),
            subject_pattern: None,
